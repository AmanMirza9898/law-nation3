...
// --- MAIN LOGIC COMPONENT (Isme useSearchParams use ho raha hai) ---
function ArticlesContent() {
  ...
  const handleSearchSubmit = (e) => {
    ...
    // Fetch function manually call karo
    fetchArticles(searchTerm);

    // URL update karo (History maintain karne ke liye)
    if (searchTerm.trim()) {
      router.push(`/articles?q=${searchTerm.trim()}`);
    } else {
      router.push(`/articles`);
...
// --- DEFAULT EXPORT WRAPPER (Fixes Build Error) ---
export default function AllArticlesPage() {
  return (
    // ✅ Suspense Boundary Add Kiya (Required for useSearchParams)
    <Suspense fallback={
